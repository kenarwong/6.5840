...
func (w *WorkerProcess) Execute() (err error) {
	...
	case MAP_TASK_TYPE:
		...
		intermediateFiles := make([]*os.File, w.workerData.task.(MapTask).nReduce)
		for r := 0; r < w.workerData.task.(MapTask).nReduce; r++ {
			oname := fmt.Sprintf(tmpPrefix+"%v-%v", w.workerData.task.Id(), r)
